<!--工厂模型多选-->
<template>
  <div :class="dialogVisible ? 'factory-model-multiple-choice' : 'factory-model-multiple-choice cur'">
    <div class="head">
      <div class="head-top">
    ...
    <div class="factory-model-multiple-choice-search">
      <div class="search">
  ...
  methods: {
      ...
      let fine = function (data) {
        data.forEach(arr => {
          if (that.isSelectAllShow) {
            if (that.notOptionalList.indexOf(arr.id) == -1 && that.notOptionalType.indexOf(arr.type) == -1) {
              arr.checked = true
    ...
    async searchDataFun() {
      ...
      if (this.searchCode) {
        let fine = function (data) {
          data.forEach(arr => {
            if (arr.name.includes(that.searchCode) || arr.id == that.searchCode) {
    ...
    async btn() {
      ...
      this.$emit("confirm", multipleTotal, breadCrumbsList);
...
.factory-model-multiple-choice-search {
  // margin: .65rem 0;
