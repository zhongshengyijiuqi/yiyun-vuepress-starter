  ...
  <div :class="dialogVisible ? 'framework' : 'framework cur'">
    ...
    <div :class="{ 'exhibit': true, 'all': isSelectAll }" v-if="!searchCode && frameworkList.length != 0">
      <ul>
        <li v-for="(item, index) in frameworkList" :key="index">
          <div class="department" v-if="!item.isWhether" @click="item.banned ? '' : personnelBtn(item)">
            <div class="not1">
              <p class="sol1"
                :style="item.banned ? 'background-color: #DEDEDE;' : !item.selection && item.checked ? 'border:0;' : ''">
                <img :src="require('./images/check.png')" alt v-if="!item.selection && item.checked" />
              </p>
              <p class="sol2" :style="item.banned ? 'color: #909399;' : ''" v-text="item.name"></p>
            </div>
          ...
          <div class="personnel" v-else @click="item.banned ? '' : personnelBtn(item)">
            <p class="sol1" :style="item.banned ? 'background-color: #DEDEDE;' : item.checked ? 'border:0;' : ''">
    ...
    <div class="search_display" v-if="searchCode && frameworkList.length != 0">
      <ul>
        <li v-for="(item, index) in frameworkList" :key="index">
          <div class="department" v-if="!item.isWhether" @click="item.banned ? '' : personnelBtn(item)">
            <div class="not1" style="width:100%;">
              <p class="sol1"
                :style="item.banned ? 'background-color: #DEDEDE;' : !item.selection && item.checked ? 'border:0;' : ''">
                <img :src="require('./images/check.png')" alt v-if="!item.selection && item.checked" />
              </p>
      ...
      <div class="delay-title">
        ...
        <ul>
          <li v-for="(item, index) in multipleTotal" :key="index">
            <p class="ellipsis" v-if="item.positions" v-text="item.name"></p>
            <p v-else>
...
export default {
  ...
  methods: {
    ...
    selectAllFun() {//全选
      ...
      this.multipleTotal = this.unique(this.multipleTotal)
    },
    personnelBtn(item) {
      let that = this
    ...
    searchDataFun() {
      ...
      if (this.searchCode) {
        ...
        this.frameworkList = this.crumbs[this.breadCrumbsList[this.breadCrumbsList.length - 1]] || []
      }
      // console.log(this.crumbs)
    },
    btn() {
      this.$emit("confirm", this.multipleTotal);
    ...
    dialogVisible() {
      if (this.dialogVisible) {
        ...
        departmentList.forEach((arr, index) => {
          ...
          arr.selection = false
          arr.checked = false
          arr.banned = this.notOptionalList.indexOf(arr.id) == -1 ? false : true
              ...
              element.deptIdList.forEach(list => {
                if (arr.id == list) {
                  let obj = {
                    ...
                    fatherName: arr.name,
                    isWhether: element.positions ? false : true,
                    selection: false,
                    id: element.id,
                    name: element.name,
        ...
        // console.log('this.frameworkList',this.frameworkList)
      } else {
        this.searchCode = ''
      }
    },
